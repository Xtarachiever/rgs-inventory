...
export const generateEmailContent = (data) => {
    const email = Object.entries(data).find(([key, val]) => key === "email")[1];
    ...
    return {
      ...
      html: `<!doctype html>
          ...
          <div>${htmlData}</div>
          <div>
          <span style="text-decoration:bold">Click this link to complete your registration: </span> <a href=${`${process.env.NEXTAUTH_URL}/complete-registration?email=${email}`}>Complete Registration</a>
          </div>
          </td>
          </tr>
          </table>
