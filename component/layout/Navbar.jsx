...
const Navbar = ({activeNav, setActiveNav, navLink}) => {
  ...
  return (
    <div className={`flex justify-between w-full p-6 px-10 items-center h-[100px] bg-white sticky top-0 z-10 shadow-xl gap-[20px] border-b border-gray ${styles.navBar}`}>
        ...
        <div className='hidden md:flex gap-[20px] items-center w-[80%] lg:w-[70%] justify-between'>
        ...
        <div className="md:hidden block">
          <div className='flex gap-[20px] items-center'>
            <IoIosNotificationsOutline  fontSize="1.7rem" cursor={'pointer'} onClick={()=>route.push('/notifications')}/>
            {
              activeNav ? 
              <IoCloseSharp fontSize="1.4rem" onClick={()=>setActiveNav(false)} cursor={'pointer'}/>
              :
              <RxHamburgerMenu fontSize="1.4rem" onClick={()=>setActiveNav(true)} cursor={'pointer'}/>
            }
          </div>
        </div>
    </div>
